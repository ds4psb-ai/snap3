#!/bin/bash
# Instagram VDP RAW Generation Pipeline - Upgraded Version
...
usage() {
    ...
    echo '  "content_id": "POST_ID",'
    echo '  "source_url": "https://instagram.com/p/POST_ID",'
...
# 메타데이터 파싱
CONTENT_ID=$(jq -r '.content_id // .post_id // "unknown"' "$METADATA_FILE")
SOURCE_URL=$(jq -r '.source_url // ""' "$METADATA_FILE")
CREATOR=$(jq -r '.creator // .username // ""' "$METADATA_FILE")

echo "📸 Instagram VDP RAW Generation Pipeline - Upgraded Version"
echo "========================================================="
...
ISHA=$(shasum -a 256 "$MP4_FILE" | cut -d' ' -f1)
echo "✅ SHA256: $ISHA"
echo ""

# 2. GCS 업로드 (비디오)
echo "☁️ 2단계: Instagram 비디오 GCS 업로드"
GCS_VIDEO_URI="gs://${RAW_BUCKET}/raw/ingest/${ISHA}.mp4"

if ! gsutil -h "x-goog-meta-vdp-upload-id:${UPLOAD_ID}" \
             -h "x-goog-meta-vdp-platform:instagram" \
...
# 3. GCS 업로드 (메타데이터)
echo "📄 3단계: Instagram 메타데이터 GCS 업로드"
GCS_META_URI="gs://${RAW_BUCKET}/raw/ingest/${ISHA}.json"
...
# 4. VDP 추출 (업그레이드된 방식 - Instagram)
echo "🧬 4단계: Instagram VDP RAW + Hook Genome 추출 (업그레이드 버전)"
echo "📞 t2-extract API 호출 중..."

VDP_OUTPUT="/tmp/vdp.instagram.${CONTENT_ID}.${TIMESTAMP}.json"

if ! curl -sS -X POST "$T2_URL/api/vdp/extract-vertex" \
          -H 'Content-Type: application/json' \
          -d "$(jq -n --arg g "$GCS_VIDEO_URI" '{gcsUri:$g, meta:{platform:"Instagram", language:"ko"}}')" \
          | tee "$VDP_OUTPUT"; then
    echo "❌ Instagram VDP 추출 실패"
    exit 1
fi
echo ""
echo "✅ Instagram VDP 추출 완료: $VDP_OUTPUT"
echo ""

# 5. Hook Gate 자동 판정
echo "🎯 5단계: Instagram Hook Gate 자동 판정"
HOOK_RESULT=$(jq '{
...
echo "Instagram Hook Gate 판정 결과:"
echo "$HOOK_RESULT" | jq '.'

HOOK_PASS=$(echo "$HOOK_RESULT" | jq -r '.pass')
if [ "$HOOK_PASS" = "true" ]; then
    echo "✅ Instagram Hook Gate 통과!"
else
    echo "❌ Instagram Hook Gate 실패!"
    echo "⚠️  start_sec ≤ 3s 그리고 strength_score ≥ 0.70 조건을 만족해야 합니다."
fi
echo ""

# 6. 로컬 파일 저장
echo "💾 6단계: 로컬 Instagram VDP 파일 저장"
LOCAL_VDP="${CONTENT_ID}_${TIMESTAMP}_INSTAGRAM_UPGRADED.vdp.json"
cp "$VDP_OUTPUT" "$LOCAL_VDP"
echo "✅ 로컬 Instagram VDP 저장: $LOCAL_VDP"
echo ""

# 7. 요약 정보 출력
echo "📊 Instagram 추출 완료 요약"
echo "=========================="
...
# 성공 메시지
if [ "$HOOK_PASS" = "true" ]; then
    echo "🎉 Instagram VDP RAW Generation Pipeline 완료! (업그레이드 버전)"
    echo "   모든 파일이 성공적으로 생성되었습니다."
else
    echo "⚠️  Instagram VDP 생성은 완료되었으나 Hook Gate를 통과하지 못했습니다."
